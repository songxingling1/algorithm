...
void __buildInOrderThread(node *root) {
     ...
     if(inOrderRoot == nullptr && root->l == nullptr ) inOrderRoot = root;
     if(root->l == nullptr) root->l = preNode,root->ltag = true;
     if(preNode != nullptr && preNode->r == nullptr) preNode->r = root,preNode->rtag = true;
     preNode = root;
